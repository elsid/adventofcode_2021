...
fn count_cave_paths(buffer: impl BufRead) -> usize {
    ...
    while let Some(path_index) = incoming.pop_front() {
        ...
        for neighbour in nodes[paths[path_index].last].iter() {
            ...
            let second_visit = if small_cave {
                if paths[path_index].visited.contains(&neighbour.as_str()) {
                    if paths[path_index].small_cave_visited_twice || neighbour == "start" {
                        continue;
                    }
                    true
                } else {
            ...
            neighbour_path.small_cave_visited_twice =
                neighbour_path.small_cave_visited_twice || second_visit;
...
"#
    .as_bytes();
    assert_eq!(count_cave_paths(buffer), 36);
}
...
"#
    .as_bytes();
    assert_eq!(count_cave_paths(buffer), 103);
}
...
"#
    .as_bytes();
    assert_eq!(count_cave_paths(buffer), 3509);
}
