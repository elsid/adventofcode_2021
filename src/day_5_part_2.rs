...
fn fill_segment(segment: &Segment, area: &mut BTreeMap<Point, usize>) {
    ...
    let direction_x = if segment.begin.x < segment.end.x {
        1
    } else {
        -1
    };
    let direction_y = if segment.begin.y < segment.end.y {
        1
    } else {
        -1
    };
    let length = (segment.end.x - segment.begin.x)
        .abs()
        .min((segment.end.y - segment.begin.y).abs());
    for shift in 0..=length {
        area.entry(Point {
            x: segment.begin.x + shift * direction_x,
            y: segment.begin.y + shift * direction_y,
        })
        .and_modify(|v| *v += 1)
        .or_insert(1);
    }
...
"#
    .as_bytes();
    assert_eq!(count_points_of_intersection(buffer), 12);
}
